...
@Client.on_callback_query(filterssᴇᴄᴛɪᴏɴ ᴛɪᴍᴇᴅ ᴏᴜᴛ 🎯.", show_alert=True)
        return ᴛɪᴍᴇᴅ ᴏᴜᴛ 🎯.", show_alert=True)

.regex(r"^next"))
async def next_page(bot, query):
    ident, req, key, offset = query.data.split("_")
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("ᴜsᴇ ʏᴏᴜʀ ᴏᴡɴ ʜᴀɴᴅs ʙʀᴏ 🧚‍♂️.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("sᴏʀʀʏ , sᴇᴄᴛɪᴏɴ ᴛɪᴍᴇᴅ ᴏᴜᴛ 🎯.", show_alert=True)
        return
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("✯ ʙᴀᴄᴋ ✯", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"❦︎ ᴘᴀɢᴇ ❦︎{round(int(offset) / 10) + 1} / {round(total / 10)}",
                                  callback_data="pages")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"❦︎ ᴘᴀɢᴇ ❦︎{round(int(offset) / 10) + 1} / {round(total / 10)}", callback_data="pages"),
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("𝐋sᴏʀʀʏ , sᴇᴄᴛɪᴏɴ ᴛɪᴍᴇᴅ ᴏᴜᴛ 🎯.".", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("ʙᴜᴅᴅʏ , ᴅᴏ ɪᴛ ʏᴏᴜʀsᴇʟғ ✨", show_alert=True)
        ...
        return await query.answer('ᴘʟᴇᴀsᴇ , sʜᴀʀᴇ ᴀɴᴅ sᴜᴘᴏʀᴛ')
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('❦︎ sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ❦︎', url='https://t.me/AboutAadhi')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="❦︎ ᴘᴀɢᴇ ❦︎ 1/1", callback_data="pages")]
        )
